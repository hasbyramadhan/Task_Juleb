...
class ProductTemplate(models.Model):
    ...
    def import_product_template_from_excel(self, data_file):
        try:
            # Baca file Excel
            excel_data = xlrd.open_workbook(file_contents=base64.b64decode(data_file))
            sheet = excel_data.sheet_by_index(0)  # Menggunakan sheet pertama

            for row_index in range(1, sheet.nrows):  # Mulai dari baris kedua (indeks 1)
                row = sheet.row_values(row_index)
                if row:
                    # Ambil data dari kolom Excel sesuai dengan header kolom
                    ...
                    # Cari produk berdasarkan Internal Reference (default_code)
                    product = self.search([('default_code', '=', internal_reference)], limit=1)
                    
                    _logger.info(f" Val product {product}")

                    if product:
                        # Update informasi produk yang sudah ada
                        _logger.info(f" Update Data Product ")
                        ...
                        # Buat produk baru jika tidak ada yang cocok
                        _logger.info(f" Create Data Product ")
                        self.create({
            ...
            return {
                'message': 'Product data has been imported successfully.',
            }

        except Exception as e:
            return {
                'error': 'An error occurred while importing product data: %s' % str(e),
            }
